...
//        }
    ...
    public String catUrl(String hash){
        try {
            ...
            return parseUrl("cat?arg=" + filePointer);
        } catch (IOException e) {
            e.printStackTrace();
            return "";
        }
    ...
    private byte[] retrieve(String path) throws IOException {
        ...
        Log.d("-------retrieve----->",protocol+"  "+host+"  "+port+"  "+version+"  "+path+"  urlStr="+target.toString());
        return IPFS.get(target);
    }


    private String parseUrl(String path) throws IOException {
        URL target = new URL(protocol, host, port, version + path);
