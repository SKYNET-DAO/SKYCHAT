...
import cn.wildfire.imshat.kit.audio.IAudioPlayListener;
import cn.wildfire.imshat.kit.conversation.message.model.UiMessage;
import cn.wildfire.imshat.kit.conversation.message.viewholder.AudioMessageContentViewHolder;
...
public class ConversationViewModel extends ViewModel implements OnReceiveMessageListener,
        ...
        OnRecallMessageListener, OnMessageUpdateListener, OnClearMessageListener, RemoveMessageListener {
    ...
    public void sendFileMsg(File file) {
        Logger.e("---------sendFileMsg---->"+file.getPath());
    ...
    public void onSendFail(Message message, int errorCode) {
        Logger.e("----chat sent  failed--onSendFail-message--->"+ JsonUtil.toJson(message));
        if (isMessageInCurrentConversation(message)) {
            UiMessage uiMessage=new UiMessage(message);
            uiMessage.setErrorCode(errorCode);//add a errorCode
            postMessageUpdate(uiMessage);
        }
    }
    ...
    public void onMessagedRemove(Message message) {
        Logger.e("------onMessagedRemove--->"+message.messageId);
        messageRemovedLiveData().setValue(new UiMessage(message));
    }
}
