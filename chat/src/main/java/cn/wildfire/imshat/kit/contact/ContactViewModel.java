...
public class ContactViewModel extends ViewModel implements OnFriendUpdateListener, AppScopeViewModel {
    ...
    public MutableLiveData<Integer> friendRequestUpdatedLiveData() {
        ...
        Log.e("ContactViewModel","------ContactViewModel---friendRequestUpdatedLiveData----->"+count);
        if (count > 0) {
    ...
    public void onFriendRequestUpdate() {
        ...
        int unreadRequestFriend=getUnreadFriendRequestCount();
        Log.e("ContactViewModel","---------onFriendRequestUpdate()----->"+unreadRequestFriend);
        if (friendRequestUpdatedLiveData != null) {
            friendRequestUpdatedLiveData.setValue(unreadRequestFriend);
        }else{
            friendRequestUpdatedLiveData = new MutableLiveData<>();
            if(unreadRequestFriend>0) friendRequestUpdatedLiveData.setValue(unreadRequestFriend);

            Log.e("ContactViewModel","---------onFriendRequestUpdate()==null----->");
        }


        
//        if(unreadRequestFriend>0){
//            EventBus.getDefault().post(unreadRequestFriend+"","ACTION_UPDATE_REQUEST_FRIEND");
//        }else{
//            EventBus.getDefault().post("ACTION_UPDATE_REQUEST_FRIEND_NOT");
//        }
