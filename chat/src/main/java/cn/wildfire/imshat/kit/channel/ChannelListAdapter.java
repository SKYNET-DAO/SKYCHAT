...
import cn.wildfire.imshat.kit.channel.viewholder.CategoryViewHolder;
import cn.wildfire.imshat.kit.channel.viewholder.ChannelViewHolder;
...
public class ChannelListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder.getItemViewType() == R.layout.channel_item_category) {
            if (position == 0) {
                ((CategoryViewHolder) holder).bind(getString(R.string.str_channel_my));
            } else {
                ((CategoryViewHolder) holder).bind(getString(R.string.str_my_subchannel));
            }
        } else {
            if (createdChannels == null || createdChannels.isEmpty()) {
