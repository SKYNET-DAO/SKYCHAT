...
import cn.wildfire.imshat.kit.conversation.message.viewholder.IntentHelper;
import cn.wildfire.imshat.kit.group.GroupViewModel;
import cn.wildfire.imshat.kit.search.OnResultItemClickListener;
import cn.wildfire.imshat.kit.search.SearchFragment;
...
public class ForwardActivity extends WfcBaseActivity {
    ...
    protected void afterViews() {
        setTitle(getString(R.string.str_transform));
//       String msgJson = getIntent().getStringExtra("message");
        message= IntentHelper.getInstance().getForwordMsg();
        IntentHelper.getInstance().setForwordMsg(null);
    ...
    private void forward(String targetName, String targetPortrait, Conversation targetConversation) {
        ...
        MaterialDialog dialog = new MaterialDialog.Builder(this)
                ...
                .positiveText(getString(R.string.str_send2))
                                ...
                                .observe(ForwardActivity.this, new Observer<OperateResult<Integer>>() {
                                    ...
                                    public void onChanged(@Nullable OperateResult<Integer> integerOperateResult) {
                                        if (integerOperateResult.isSuccess()) {
                                            Toast.makeText(ForwardActivity.this, UIUtils.getString(R.string.str_transform_success), Toast.LENGTH_SHORT).show();
                                            finish();
                                        } else {
                                            Toast.makeText(ForwardActivity.this, UIUtils.getString(R.string.str_transform_fail) + integerOperateResult.getErrorCode(), Toast.LENGTH_SHORT).show();
                                        }
                                    }
                                });
